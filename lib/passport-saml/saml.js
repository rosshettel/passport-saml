...
SAML.prototype.validateSignature = function (fullXml, currentNode, cert) {
  ...
  if (signatures.length != 1) {
      console.log('signatures.length != 1 line 346');
    return false;
  }
  ...
  if (sig.references.length != 1 ) {
      console.log('sig.references.length != 1 line 364');
      return false;
  }
  ...
  console.log('idAttribute', currentNode.getAttribute('ID'));
  console.log('currentNode.getAttribute', currentNode.getAttribute(idAttribute));
  console.log('refId', refId);
  if (currentNode.getAttribute(idAttribute) != refId) {
      console.log('currentNode idAttribute != refId line 373');
      return false;
  }
  ...
  if (totalReferencedNodes.length > 1) {
      console.log('totalReferencedNodes.length > 1 line 382');
      return false;
  }
  console.log('sig.checkSignature(fullXml) line 385', sig.checkSignature(fullXml));
...
SAML.prototype.validatePostResponse = function (container, callback) {
  ...
  .then(function() {
    ...
    if (assertions.length + encryptedAssertions.length > 1) {
      ...
      throw new Error('Invalid signature - line 415');
    }
    ...
    if (assertions.length == 1) {
      if (self.options.cert &&
        ...
        throw new Error('Invalid signature - line 422');
      }
    ...
    if (encryptedAssertions.length == 1) {
      ...
      if (encryptedDatas.length != 1)
        throw new Error('Invalid signature - line 433');
      ...
      return Q.ninvoke(xmlenc, 'decrypt', encryptedDataXml, xmlencOptions)
        .then(function(decryptedXml) {
          ...
          if (self.options.cert &&
            ...
            throw new Error('Invalid signature - line 447');
    ...
    return Q.ninvoke( parser, 'parseString', xml)
      .then(function(doc) {
        ...
        if (response) {
              ...
              if (statusCode && statusCode[0].$.Value === "urn:oasis:names:tc:SAML:2.0:status:Responder") {
                ...
                if (nestedStatusCode && nestedStatusCode[0].$.Value === "urn:oasis:names:tc:SAML:2.0:status:NoPassive") {
                  if (self.options.cert && !validSignature) {
                    throw new Error('Invalid signature - line 475');
                  }
          ...
          if (self.options.cert && !validSignature) {
            throw new Error('Invalid signature - line 507');
          }
...
SAML.prototype.validatePostRequest = function (container, callback) {
  ...
  parser.parseString(xml, function (err, doc) {
    ...
    if (self.options.cert && !self.validateSignature(xml, self.options.cert)) {
      return callback(new Error('Invalid signature - line 708'));
    }
